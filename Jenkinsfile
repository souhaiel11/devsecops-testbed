                    ...
                    withSonarQubeEnv('sq1') {
                        withCredentials([string(credentialsId: 'SONAR_TOKEN', variable: 'SONAR_TOKEN')]) {
                            script {
                                ...
                                sh '''
                                    set -e
                                    export SONAR_TOKEN_VAL="$SONAR_TOKEN"
                                    mvn sonar:sonar -B \\
                                      -DskipTests=true \\
                                      -Djacoco.skip=true \\
                                ...
                                ''' + (prArgs ? "\n                                    sh 'mvn sonar:sonar -B -DskipTests=true -Djacoco.skip=true -Dsonar.projectKey=\"\$APP_NAME\" -Dsonar.projectName=\"Devsecops Testbed\" -Dsonar.host.url=\"\$SONAR_HOST_URL\" -Dsonar.token=\"\$SONAR_TOKEN_VAL\"'" : '')
                            }
                        }
                    }
                }
            ...
            parallel {
                ...
                stage('OWASP Dependency Check') {
                    ...
                    steps {
                        ...
                        catchError(buildResult: 'UNSTABLE', stageResult: 'FAILURE') {
                            withCredentials([string(credentialsId: 'NVD_API_KEY', variable: 'NVD_API_KEY_VAL')]) {
                                sh '''
                                    set -e
                                    ODC_VERSION="12.2.2"
                                    ...
                                    export NVD_API_KEY="$NVD_API_KEY_VAL"
